...
export function getAverage(...numbers) {
  numbers = [6, 2, 55, 11, 78, 2.5, 55, 77, 5.7, 87, 23, 2, 5.6, 3, -2];

  let res = numbers.filter((item) => Number.isInteger(item));

  let result = res.reduce((sum, current) => sum + current) / res.length;

  const output = document.querySelector(".hw-5");
...
export function getSubjects(students) {
  ...
  const output = document.querySelector(".hw-6");
  output.innerHTML = `${subNames}`;
}
...
function getMyTaxes(salary) {
  ...
  // return taxes;
  const output = document.querySelector(".hw-7");
  output.innerHTML = `${taxes}`;
}
...
const output = document.querySelector(".hw-8");
output.innerHTML = `${person.getInfo()}`;
...
export function getRandomColor() {
  ...
  const output = document.querySelector(".hw-9");
...
export function playSound(e) {
  ...
  const key = document.querySelector(`.key[data-key = "81"]`);
  //if (!audio) return;
  //audio.play();
  //audio.currentTime = 0;
  console.log(key);
  const output = document.querySelector(".hw-10");
